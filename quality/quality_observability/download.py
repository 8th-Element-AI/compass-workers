"""HuggingFace download for `quality-score download`.

Walks every entry under `models:` in the runtime.yaml and pulls the
matching HF repo to its `local_path`. Mirrors toxicity_observability.download
so the Docker build stage looks identical across the two packages.
"""
...
def download_models(config_path: str | None = None) -> dict[str, str]:
    ...
    Reads `HF_TOKEN` (or `HUGGING_FACE_HUB_TOKEN`) from env for private
    repos. Entries without `repo_id` are skipped with a warning — useful
    when a model has been side-loaded and you only want to verify the
    others.
    """
