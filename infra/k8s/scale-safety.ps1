<#
.SYNOPSIS
    Scale the Safety worker StatefulSet, with automatic slot rebalancing.
...
.PARAMETER PgPodLabel
    K8s pod label selector for the Postgres pod (for verification).
    Default: app=postgres,lens=signal.

.EXAMPLE
    .\scale-safety.ps1 -Replicas 4
    Scales safety to 4 pods.

.EXAMPLE
    .\scale-safety.ps1 -Replicas 8
    Scales safety to 8 pods (each owns 2 slots).
#>
param(
    ...
    [string]$StatefulSet = "signal-worker-safety",
    [int]$TotalSlots = 16,
...
$pods = kubectl get pods -n $Namespace -l "app=signal-worker,lens=safety" `
...
while ((Get-Date) -lt $deadline -and $missing.Count -gt 0) {
    $actual = kubectl exec -n $Namespace $pgPodName -- `
        ...
        -c "SELECT partition_key FROM worker_checkpoints WHERE lens='safety' ORDER BY partition_key" 2>$null
...
if ($missing.Count -eq 0) {
    ...
    kubectl get pods -n $Namespace -l "app=signal-worker,lens=safety"
    exit 0
} else {
    Write-Warning "`nSome expected slots missing after ${VerifyTimeoutSec}s. Missing: $($missing -join ', ')"
    Write-Warning "Likely cause: those slots haven't received any spans yet (worker hasn't checkpointed them)."
    Write-Warning "Check ``kubectl logs`` on the relevant pods; the rebalancing itself is complete."
    exit 1
}