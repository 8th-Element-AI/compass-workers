...
signal_worker/
  ...
  scorers.py     SHARED  — QualityScorer interface + local-model implementation
  lenses/
    performance.py  LENS — 17 specs + build_context
    cost.py         LENS — 22 specs + build_context (+ span_types filter)
    safety.py       LENS — PII specs + batched Presidio + content-hash cache
    quality.py      LENS — 11 P0 specs: mechanical checks + semantic scoring
```
...
### 5.3 Safety — PII detection (Presidio)

Processes `model_call` spans only. Each batch's input/output texts are deduped
by content hash, analyzed in **one** batched Presidio call, and cached in an
in-process LRU so repeated prompts (the common case) aren't re-analyzed. Emits
`pii_count`, `pii_detected`, `pii_distinct_types`; the detected entity types and
which side they appeared on (input/output/both) ride along in `metric_meta` via
the generic `meta_fn` hook.

### 5.4 Quality — 11 P0 metrics (mechanical + semantic)

The first **semantic** lens. Reads `model_call`, `tool_call`, `retrieval`,
`validation`, and `skill_exec` spans (pushed into the fetch query via
`span_types`). Two compute classes inside one lens:

- **Mechanical** (pure metadata checks): `schema_conformance`,
  `format_correctness`, `tool_call_validity`, `data_completeness`,
  `data_accuracy`, and `constraint_satisfaction` (computed only when the
  runtime records a `metadata.constraints` object — a documented convention,
  nothing emits until instrumentation adopts it).
- **Semantic** (local models via the pluggable `QualityScorer` in
  `scorers.py`): `faithfulness`, `coherence`, `completeness` on model calls;
  `context_relevance`, `chunk_utilization` on retrievals. Scoring follows the
  Safety pattern — per-batch dedupe by content hash, one batched model call per
  job kind, LRU cache. `SIGNAL_QUALITY_SAMPLE` scores a deterministic subset
  when full coverage is too expensive; `SIGNAL_QUALITY_SEMANTIC=false` turns
  the lens into a cheap mechanical-only worker (no torch).

Two honest caveats, by design:

- The semantic scores are **local-model proxies** (NLI entailment, embedding
  coverage, relevance cross-encoders), not LLM-judge scores — coarser but free
  and self-hosted. The `QualityScorer` interface is the seam where an LLM judge
  can be swapped in later without touching the specs.
- `chunk_utilization` is the system's first **cross-span** metric: a retrieval
  span's chunks are compared against the same-trace `model_call` output. The
  trace→answer map is built per batch, so it's best-effort — if a trace is split
  across batch boundaries the metric is simply not emitted for that span.

Scope notes: `drift_score` (batch/baseline comparison) and
`context_recall`/`context_precision` (need ground-truth relevance labels) are
...
- **Semantic scorer fidelity** — the `QualityScorer` interface exists and ships
  with local-model proxies (§5.4); an LLM-judge implementation (higher fidelity,
  per-call cost) is the natural upgrade behind the same interface.
