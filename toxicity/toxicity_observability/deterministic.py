...
# ---- Attack (prompt injection) patterns ----------------------------------
_ATTACK_PATTERNS = [
...
# ---- Moderation patterns — split by target sub-label ---------------------
# So rule short-circuit can set the right score(s) without guessing.
_HARMFUL_PATTERNS = [
...
# CSAM patterns trigger BOTH harmful and sexual.
_CSAM_PATTERNS = [
...
@dataclass
class GateResult:
    """Output of the deterministic gate.

    force_route : routes (ATTACK / MODERATION) the rules want activated —
                  used by the worker for diagnostics / metric_meta.
    force_label : public-facing labels to set to 1.0 when the rule fires.
                  This is what the pipeline uses to short-circuit BERT.
    reasons     : human-readable rule names for diagnostics.
    """
    force_route: set[str]  = field(default_factory=set)
    force_label: set[str]  = field(default_factory=set)
...
def evaluate(norm: NormalizedText) -> GateResult:
    ...
    if _hit(_ATTACK_PATTERNS, text):
        ...
        result.force_label.add(C.PROMPT_INJECTION)
        result.reasons.append("rule:attack")

    if _hit(_HARMFUL_PATTERNS, text):
        result.force_route.add(MODERATION)
        result.force_label.add(C.HARMFUL_CONTENT)
        result.reasons.append("rule:harmful")

    if _hit(_SEXUAL_PATTERNS, text):
        result.force_route.add(MODERATION)
        result.force_label.add(C.SEXUAL)
        result.reasons.append("rule:sexual")

    if _hit(_CSAM_PATTERNS, text):
        result.force_route.add(MODERATION)
        result.force_label.add(C.HARMFUL_CONTENT)
        result.force_label.add(C.SEXUAL)
        result.reasons.append("rule:csam")
