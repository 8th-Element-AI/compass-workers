...
from compass_worker.config import Config
from compass_worker.lenses.cost import CostWorker
...
def _run_all(cfg: Config, once: bool) -> int:
    ...
    try:
        ...
        # while threads are still running (a plain t.join() would block compass).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compass Lens Worker Manager")
    parser.add_argument(
        "--worker",
...
def _derive_partition_index_from_pod_name():
    ...
    StatefulSet pods get stable ordinal names: compass-worker-safety-2 etc.
