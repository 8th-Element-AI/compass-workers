...
`compute(span) -> list[MetricRow]`. The base class handles:
  ...
  * writing computed metric rows into `compass_derived_metrics`,
...
log = logging.getLogger("compass.worker")
...
def path_cols(span: dict, scope: str) -> dict:
    ...
    Matches the materialized-path convention used everywhere in Compass:
...
class BaseWorker:
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self._ch = None
        # Shutdown compass — set by stop() to terminate run_poll cleanly between batches.
    ...
    # ---- shutdown compass (used by main process on Ctrl+C / SIGTERM) ----
    def stop(self):
