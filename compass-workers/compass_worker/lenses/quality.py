...
Semantic scoring can be disabled (COMPASS_QUALITY_SEMANTIC=0 -> mechanical
metrics only; no torch required) and sampled (COMPASS_QUALITY_SAMPLE=0.2 scores a
...
log = logging.getLogger("compass.worker.quality")
...
class QualityWorker(SpecWorker):
    ...
    def __init__(self, cfg, scorer=None):
        ...
        self.semantic = bool(getattr(cfg, "compass_quality_semantic", True))
        self.sample = float(getattr(cfg, "compass_quality_sample", 1.0))
        self.cache_max = getattr(cfg, "compass_quality_cache_max", 20000)
        self._scorer = scorer
    ...
    def scorer(self):
        if self._scorer is None:
            ...
            log.info(
                ...
                getattr(self.cfg, "compass_quality_models_root", "./models"),
            )
    ...
    def _quality_config(self) -> dict:
        ...
        `compass_quality_models_root`. Same pattern SafetyWorker uses for
        ...
        """
        c = self.cfg
        root = Path(getattr(c, "compass_quality_models_root", "./models"))
        ...
        return {
            "models": {
                "nli": {
                    "local_path": _resolve(getattr(c, "compass_quality_nli_path", "nli")),
                },
                "embedding": {
                    "local_path": _resolve(getattr(c, "compass_quality_embed_path", "embedding")),
                },
                "relevance": {
                    "local_path": _resolve(getattr(c, "compass_quality_relevance_path", "relevance")),
                },
            },
            "recipes": {
                "premise_max_chars": getattr(c, "compass_quality_premise_max_chars", 2000),
                "max_sents":         getattr(c, "compass_quality_max_sents", 10),
                "sent_min_chars":    getattr(c, "compass_quality_sent_min_chars", 3),
                "chunk_used_cos":    getattr(c, "compass_quality_chunk_used_cos", 0.5),
            },
            "runtime": {
                "device":     getattr(c, "compass_quality_device", "cpu"),
                "batch_size": getattr(c, "compass_quality_batch", 32),
            },
        }
    ...
    def _verify_models_ready(self) -> None:
        ...
        artifacts = [
            ("nli",       _resolve(getattr(c, "compass_quality_nli_path", "nli"))),
            ("embedding", _resolve(getattr(c, "compass_quality_embed_path", "embedding"))),
            ("relevance", _resolve(getattr(c, "compass_quality_relevance_path", "relevance"))),
        ]
