...
`performance`, `cost`, `safety`, and `quality`. Planned: `outcomes`.
...
└── signal_worker/
    ...
    ├── scorers.py           QualityScorer — local-model semantic scoring (Quality lens)
    └── lenses/
        ├── performance.py   Performance lens — 17 metrics
        ├── cost.py          Cost lens — 22 metrics
        ├── safety.py        Safety lens — PII detection via Presidio (batched)
        └── quality.py       Quality lens — 11 P0 metrics (mechanical + semantic)
```
...
| `SIGNAL_QUALITY_SEMANTIC` | `true` | quality | `false` = mechanical metrics only (no torch needed) |
| `SIGNAL_QUALITY_SAMPLE` | `1.0` | quality | fraction of spans semantically scored (deterministic by span_id) |
| `SIGNAL_QUALITY_NLI_MODEL` | `MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli` | quality | faithfulness / coherence model (FEVER-trained; benchmarked best) |
| `SIGNAL_QUALITY_EMBED_MODEL` | `sentence-transformers/all-MiniLM-L6-v2` | quality | completeness / chunk-utilization embeddings |
| `SIGNAL_QUALITY_RELEVANCE_MODEL` | `cross-encoder/ms-marco-MiniLM-L-6-v2` | quality | context-relevance model |
| `SIGNAL_QUALITY_BATCH` | `32` | quality | model inference batch size |
| `SIGNAL_QUALITY_CACHE_MAX` | `20000` | quality | LRU cap on the scoring cache |
